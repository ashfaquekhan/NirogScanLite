...
def adc_to_uv(adc_val):
    centered = adc_val - 2048
    uv = (centered * ADC_VREF_MV * 1000.0) / (ADC_MAX_VALUE * AD8232_GAIN)
    return uv
...
class NirogScanGUI(QMainWindow):
    def __init__(self):
        ...
        self.ecg_buffer = CircularBuffer(PLOT_WINDOW, dtype=np.float32)
    ...
    def setup_ui(self):
        ...
        self.ecg_plot.setRange(xRange=[0, PLOT_WINDOW], yRange=[-1500, 1500])
    ...
    def on_new_packet(self, packet):
        ...
        ecg_uv = [adc_to_uv(adc) for adc in packet.ecg]
